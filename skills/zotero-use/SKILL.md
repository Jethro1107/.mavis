...
5. Do not add MCP config to `.agents`, skill metadata, or agent config by default.
...
3. For a single selected item, check children/attachments and report whether a PDF is available (see `references/finding-attachments.md` for how to locate the file path).
...
- Finding attachment file paths (linked, imported, URL): `references/finding-attachments.md`
